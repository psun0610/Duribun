...
6. Request `profile_nickname` and `profile_image`. Request `account_email` only if the Kakao app is eligible; otherwise enable Supabase's setting that allows users without email for Kakao.
